...
func TestGetUUIDsAndObjectKeysByObjectKeys(t *testing.T) {
	type args struct {
		title     string
		setupMock func() *sql.DB
		wantErr   bool
	...
	testScenarios := []args{
		{
			...
			setupMock: func() *sql.DB {
				db, mock, _ := sqlmock.New()
				placholder := "$1"
				expectedQuery := fmt.Sprintf("SELECT o.object_key,o.object_uuid FROM object_versions ov INNER JOIN objects o ON o.id = ov.object_id WHERE o.object_key IN ( %s)  AND o.bucket_id = $%d", placholder, 2)
				mock.ExpectPrepare(regexp.QuoteMeta(expectedQuery)).
					ExpectQuery().
					WithArgs(sqlmock.AnyArg(), sqlmock.AnyArg()).
					WillReturnRows(sqlmock.NewRows([]string{"object_key", "object_uuid"}).AddRow("test", "1234"))
	...
	for _, testScenario := range testScenarios {
		t.Run(testScenario.title, func(t *testing.T) {
			...
			_, err := repo.GetUUIDsAndObjectKeysByObjectKeys(ctx, 1, []string{"1"})
			if (err != nil) != testScenario.wantErr {
				t.Errorf("GetUUIDsAndObjectKeysByObjectKeys() error = %v, wantErr = %v", err, testScenario.wantErr)
			}

			if err != nil && testScenario.err != nil {
				if err.Error() != testScenario.err.Error() {
					t.Errorf("GetUUIDsAndObjectKeysByObjectKeys() error = %v, scenarioError = %v", err, testScenario.err)
				}
			}
		})
	}
