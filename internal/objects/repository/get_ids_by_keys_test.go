...
func TestGetIDsByKeys(t *testing.T) {
	type args struct {
		title     string
	...
	for _, testscenario := range testScenarios {
		t.Run(testscenario.title, func(t *testing.T) {
			...
			_, err := repo.GetIDsByKeys(ctx, []string{"test"}, 1)

			if (err != nil) != testscenario.wantErr {
				t.Errorf("GetIDsByKeys() error = %v, wantErr = %v", err, testscenario.wantErr)
			}

			if err != nil && testscenario.err != nil {
				if err.Error() != testscenario.err.Error() {
					t.Errorf("GetIDsByKeys() error = %v, scenarioError = %v", err, testscenario.err)
				}
			}
		})
	}
